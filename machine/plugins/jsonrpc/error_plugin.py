...
from machine.exceptions.plugins.jsonrpc import (
    MachineJsonRPCRequestError,
    ...
    async def __call__(self, conn: Connection, params: Parameters) -> PluginResult:
        try:
            ...
            if isinstance(error, MachineJsonRPCRequestError):
                await conn.send_text(body=error.message, status_code=error.status_code)
                ...
                await conn.send_json(
                    body={
                        ...
                        "error": {"message": error.message, "status": error.error_code},
                    },
